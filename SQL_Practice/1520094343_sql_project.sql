...
SELECT count(case when membercost=0 then name else null end) as no_membercost
FROM Facilities 
...
join Members on Bookings.memid=Members.memid
Where ((Bookings.memid=0) and (Facilities.guestcost * Bookings.slots>30))
or ((Bookings.memid!=0)) and ((Facilities.membercost * Bookings.slots>30))
ORDER BY cost DESC

/* Q9: This time, produce the same result as in Q8, but using a subquery. */

select Distinct concat(sub.firstname, ' ',sub.surname) as fullname,
	sub.name,
	sub.cost
...
select *
From(Select Facilities.name,
		sum(case when Bookings.memid=0 then Facilities.guestcost * Bookings.slots Else Facilities.membercost * Bookings.slots end) as revenue
	from Facilities
	join Bookings on Facilities.facid=Bookings.facid
	join Members on Bookings.memid=Members.memid
